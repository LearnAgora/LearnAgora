...
//        $sql = "SELECT le.id FROM LearningEntity le left outer join (SELECT o.learning_entity_id as leid, t.user_id as uid FROM Outcome o, Trace t WHERE o.id=t.outcome_id AND t.user_id=? group by o.learning_entity_id) a on le.id=a.leid  WHERE le.discr='action' and a.uid IS NULL ORDER BY RAND() LIMIT 1";
    ...
    public function findOneOrNullUnvisitedActionsForAgora(User $user, AgoraBase $agora)
    {
        ...
        if (is_a($agora,'La\CoreBundle\Entity\Agora')) {
        ...
        if (is_a($agora,'La\CoreBundle\Entity\Techne')) {
            $sql = "SELECT * from (select le.id as id, (select count(*)  from Outcome o, Trace t where t.user_id=? and t.outcome_id=o.id and o.learning_entity_id=le.id) as cnt from LearningEntity le, Uplink u, Uplink v where le.discr='action' and u.child_id=le.id and u.parent_id=v.child_id and v.parent_id=? ) res where cnt=0 order by rand() limit 1";
        }
        $query = $this->getEntityManager()->createNativeQuery($sql, $rsm);
        $query->setParameter(1, $user->getId());
    ...
    public function findOneOrNullPostponedActionsForAgora(User $user, AgoraBase $agora){
        ...
        if (is_a($agora,'La\CoreBundle\Entity\Techne')) {
            $sql = "select * from (select le.id as id, (select o.caption from Outcome o, Trace t where t.user_id=? and t.outcome_id=o.id and o.learning_entity_id=le.id order by t.created_time desc limit 1) as caption from LearningEntity le, Uplink u, Uplink v where le.discr='action' and u.child_id=le.id and u.parent_id=v.child_id and v.parent_id=?) res where res.caption='LATER' order by rand() limit 1";
        }
        $query = $this->getEntityManager()->createNativeQuery($sql, $rsm);
        $query->setParameter(1, $user->getId());
        $query->setParameter(2, $agora->getId());
